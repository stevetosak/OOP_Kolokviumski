...
public:
    ...
    virtual int getBodovi(){
        ...
        float div = count/(brojOcenki *1.0);
        return div * 100;
    }
...
public:
    ...
    Demonstrator(int indeks, int* ocenki, int br_ocenki,char* imeprezime,Kurs* kursevi, int broj_kursevi, int broj_casovi) :
    Student(indeks,ocenki,br_ocenki),Predavach(imeprezime, kursevi,broj_kursevi)
    {
        this->br_casovi = broj_casovi;
    }
    ...
    int getBodovi(){
        try
        {
            ...
            int q = (20 * br_casovi) / brojKursevi;
            return Student::getBodovi() + q;
        }
        catch(const char* msg)
        {
            cout << "Demonstratorot so indeks " << indeks <<
            " ne drzi laboratoriski vezbi" << endl;
            return 0;
        }
    }
};
